...
GrammarCompiler::GrammarCompiler()
{
	...
	////////////////////////	t_type	/////////////////////////////////
		//basic_types
			//byte
				sbyte = as_xpr("signed") >> +_s >> "char";
				byte = as_xpr("unsigned") >> +_s >> "char";
				t_byte = sbyte | byte;
			//integer
				int_signed = !(as_xpr("signed") >> +_s) >> "int";
				int_unsigned = as_xpr("unsigned") >> +_s >> "int";
				sint_signed = !(as_xpr("signed") >> +_s) >> "short" >> +_s >> "int";
				sint_unsigned = as_xpr("unsigned") >> +_s >> "short" >> +_s >> "int";
				lint_signed = !(as_xpr("signed") >> +_s) >> (as_xpr("long") | "long long") >> +_s >> "int";
				lint_unsigned = as_xpr("unsigned") >> +_s >> (as_xpr("long") | "long long") >> +_s >> "int";
				
				t_int = int_signed | int_unsigned | sint_signed | sint_unsigned | lint_signed | lint_unsigned;
			//float
				t_float = as_xpr("float");
			//double
				t_double =  as_xpr("double") | "long double";
			//char
				t_char = as_xpr("char") | "w_char_t" | "char16_t" | "char32_t";
			//bool
				t_bool = as_xpr("bool");
			
		t_basic = t_byte | t_int | t_float | t_double | t_char | t_bool;
	...
	///////////////////////// EXPRESSIONS ///////////////////////////////
		...
		primary_expr = (_b >> as_xpr("this") >> _b) | keep(idvar) | keep(literal) | keep(primary_expr_pars);
		...
		new_array = _b >> as_xpr("new")>> _b >> *_s >> (s1= t_type) >> *_s >> !(s2= t_pointer) >> *_s >> (s3= arr_part) >> *_s >> !(s4 = by_ref(initializer));
		...
		decl_qualifier = _b >> as_xpr("const") >> _b;				// quailifier
		...
		func_def =  (s1= ret_val) >> *_s >> (s2= decl_id) >> *_s >> '(' >> *_s >> !(s3= param_list) >> *_s >> ')' >> spc >> (s4= by_ref(comp_stat));
		...
		///// labeled statement
			lbl_case = _b >> as_xpr("case") >> _b >> *_s >> (s1= const_expr) >> *_s >> ':' >> spc >> (s2= by_ref(statement));
			lbl_def = _b >> as_xpr("default") >> _b >> *_s >> ':' >> spc >> (s1= by_ref(statement));
		...
		///// compound statement
		comp_stat = as_xpr('{') >> spc >> (s1= *(by_ref(statement) >> spc)) >> spc >> '}';							
		...
		///// for statement
		for_init_stat = (s1= (declaration | expression)) >> *_s >> ";";
		...
		jump_return = _b >> as_xpr("return") >> _b >> *_s >> !(s1= expression) >> *_s >> ';';
		jump_continue = _b >> as_xpr("continue") >> _b >> *_s >> ';';
		jump_break = _b >> as_xpr("break") >> _b >> *_s >> ';';
